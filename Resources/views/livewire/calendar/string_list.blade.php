<div class="select-none">
    ...
    <div class="flex justify-between items-center mb-8">
        <button type="button" wire:click="setByMonth({{ $currentMonth - 1 }})" class="
                flex items-center mr-3 focus:outline-none
                @if ($this->showPreviousArrow()) text-gray-800 cursor-pointer @endif
            @if (!$this->showPreviousArrow()) text-gray-400 cursor-default
            @endif
            "
        ...
        <div class="flex flex-no-wrap justify-between">
            ...
            @foreach ($days as $day)
                <div class="flex justify-center w-1/7 py-1">
                    <button type="button" wire:click="setByDay({{ $day['day'] }})" class="
                            ...
                            @if ($day['current'] && !$day['selected']) border-gray-400 hover:border-gray-400 @endif @if ($day['disabled']) text-gray-400 hover:border-white cursor-default
            @endif
