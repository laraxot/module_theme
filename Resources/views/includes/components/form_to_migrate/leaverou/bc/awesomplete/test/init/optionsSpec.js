describe(
    "Constructor options", function () {
        ...
        subject(
            function () {
                return new Awesomplete(this.element, this.options) }
        );
        ...
        describe(
            "with default options", function () {
                ...
                it(
                    "requires minimum 2 chars to open completer", function () {
                        expect(this.subject.minChars).toBe(2);
                    }
                );

                it(
                    "shows 10 items in completer", function () {
                        expect(this.subject.maxItems).toBe(10);
                    }
                );

                it(
                    "does not select the first ocurrence automatically" , function () {
                        expect(this.subject.autoFirst).toBe(false);
                    }
                );

                it(
                    "modifies list item with DATA", function () {
                        expect(this.subject.data).toBe(Awesomplete.DATA);
                    }
                );

                it(
                    "filters with FILTER_CONTAINS", function () {
                        expect(this.subject.filter).toBe(Awesomplete.FILTER_CONTAINS);
                    }
                );

                it(
                    "orders with SORT_BYLENGTH", function () {
                        expect(this.subject.sort).toBe(Awesomplete.SORT_BYLENGTH);
                    }
                );

                it(
                    "creates item with ITEM", function () {
                        expect(this.subject.item).toEqual(Awesomplete.ITEM);
                    }
                );

                it(
                    "replaces input value with REPLACE", function () {
        ...
        describe(
            "with custom options in constructor", function () {
                def("element", "#with-data-list");
                def(
                    "options", function () {
                        return {
                ...
                it(
                    "overrides simple default options", function () {
                ...
                it(
                    "overrides default functions", function () {
        ...
        describe(
            "with custom options in data-* attributes", function () {
