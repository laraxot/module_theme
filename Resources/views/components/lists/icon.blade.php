<div class="cmp-icon-list">
  @if(isset($title))
  <h2 class="title-xxlarge mt-40 mb-2 mb-lg-4 {{$titleClass}}" @isset($id_title) id="{{$id_title}}"@endif>{{{$title}}}</h2>
  @endif
    ...
    <li class="{{class}}">
      <a class="list-item icon-left t-primary title-small-semi-bold" href="#" @if(isset($aria_label)) aria-label="{{$item->aria_label}} {{$item->link}}"@endif @if(isset($inly_ariaLabel)) aria-label="{{$item->only_ariaLabel}}"@endif>
        <span class="list-item-title-icon-wrapper">
          <svg class="icon icon-sm align-self-start {{$item->icon_class}} icon-color" aria-hidden="true"><use href="../assets/bootstrap-italia/dist/svg/sprites.svg#{{$item->icon}}"></use></svg>
