...
(function () {
    ...
    var unwrap = function (data) {
        var unwrapped = merge(
            data, {
                ...
                source2: get(data, 'source2').bind(
                    function (source2) {
                        return get(source2, 'value');
                    }
                ).getOr(''),
            poster: get(data, 'poster').bind(
                function (poster) {
    ...
    var submitForm = function (prevData, newData, editor) {
        ...
        if (newData.embed && (prevData.source1 === newData.source1 || Service.isCached(newData.source1))) {
            ...
            Service.getEmbedHtml(editor, newData).then(
                function (response) {
    ...
    var setup = function (editor) {
        editor.on(
            'preInit', function () {
                ...
                global$1.each(
                    'video audio iframe object'.split(' '), function (name) {
                        specialElements[name] = new RegExp('</' + name + '[^>]*>', 'gi');
                    }
                );
                var boolAttrs = editor.schema.getBoolAttrs();
                global$1.each(
                    'webkitallowfullscreen mozallowfullscreen allowfullscreen'.split(' '), function (name) {
                        boolAttrs[name] = {};
                    }
                );
                ...
                editor.serializer.addAttributeFilter(
                    'data-mce-object', function (nodes, name) {
                        var i = nodes.length;
                        var node;
                        var realElm;
                        var ai;
