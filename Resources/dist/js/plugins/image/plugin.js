...
(function (domGlobals) {
        ...
        var withTimeout = function (timeout, errorThunk) {
            return wrap(
                Future.nu(
                    function (callback) {
                            var timedOut = false;
                            var timer = domGlobals.setTimeout(
                                function () {
                                    timedOut = true;
    ...
    var makeItems = function (info) {
        ...
        var imageList = info.imageList.map(
            function (items) {
                return {
                    name: 'images',
                    type: 'selectbox',
        ...
        var classList = info.classList.map(
            function (items) {
                return {
                    name: 'classes',
                    type: 'selectbox',
    ...
    var write = function (normalizeCss, newData, image) {
        ...
        updateProp(
            image, oldData, newData, 'caption', function (image, _name, _value) {
        ...
        updateProp(
            image, oldData, newData, 'style', normalized(
    ...
    var splitTextBlock = function (editor, figure) {
        ...
        var textBlock = dom.getParent(
            figure.parentNode, function (node) {
    ...
    var collect = function (editor) {
        var urlListSanitizer = ListUtils.sanitizer(
            function (item) {
                return editor.convertURL(item.value || item.url, 'src');
            }
        );
        var futureImageList = Future.nu(
            function (completer) {
                Utils.createImageList(
        ...
        var prependURL = Option.some(Settings.getPrependUrl(editor)).filter(
            function (preUrl) {
                return isString(preUrl) && preUrl.length > 0;
            }
        );
        return futureImageList.map(
            function (imageList) {
                return {
                    image: image,
                    imageList: imageList,
    ...
    var changeImages = function (helpers, info, state, api) {
        ...
        image.each(
            function (img) {
                var updateAlt = data.alt === '' || state.prevImage.map(
                    function (image) {
    ...
    var changeFileInput = function (helpers, info, state, api) {
        ...
        head(data.fileinput).fold(
            function () {
                ...
                Utils.blobToDataUri(file).then(
                    function (dataUrl) {
                        var blobInfo = helpers.createBlobCache(file, blobUri, dataUrl);
                        uploader.upload(blobInfo).then(
                            function (url) {
                                api.setData(
                                    {
                                        src: {
    ...
    var imageSize = function (editor) {
        return function (url) {
            return FutureResult.nu(
                function (completer) {
                    Utils.getImageSize(
                        editor.documentBaseURI.toAbsolute(url), function (data) {
                            var result = data.map(
                                function (dimensions) {
                                    return {
    ...
    var Dialog = function (editor) {
        ...
        var open = function () {
            return collect(editor).map(makeDialog(helpers)).get(
                function (spec) {
