...
(function () {
    ...
    var getSelectors = function (editor, doc, fileFilter) {
        ...
        function append(styleSheet, imported)
        {
            ...
            global$4.each(
                styleSheet.imports, function (styleSheet) {
            ...
            global$4.each(
                rules, function (cssRule) {
                    if (cssRule.styleSheet) {
                        append(cssRule.styleSheet, true);
                    } else if (cssRule.selectorText) {
                        global$4.each(
                            cssRule.selectorText.split(','), function (selector) {
        ...
        try {
            global$4.each(
                doc.styleSheets, function (styleSheet) {
                    append(styleSheet);
                }
            );
        } catch (e) {
    ...
    var setup = function (editor) {
        editor.on(
            'init', function (e) {
                ...
                global$4.each(
                    getSelectors(editor, editor.getDoc(), compileFilter(Settings.getFileFilter(editor))), function (selector) {
                        if (selector.indexOf('.mce-') === -1) {
                            if (!selectorFilter || selectorFilter(selector)) {
                                var selectorGroups = getGroupsBySelector(groups, selector);
                                if (selectorGroups.length > 0) {
                                    global$4.each(
                                        selectorGroups, function (group) {
