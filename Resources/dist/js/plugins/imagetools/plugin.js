...
(function (domGlobals) {
    ...
    function create$1(getCanvas, blob, uri)
    {
        ...
        {
            return toAdjustedDataURL(type, quality).then(
                function (dataurl) {
    ...
    {
        return canvasToBlob(canvas, type).then(
            function (blob) {
    ...
    var getHttpErrorMsg = function (status) {
        var message = find(
            friendlyHttpErrors, function (error) {
                return status === error.code;
            }
        ).fold(
            constant('Unknown ImageProxy error'), function (error) {
    ...
    var getServiceErrorMsg = function (type) {
        return find(
            friendlyServiceErrors, function (error) {
                return error.type === type;
            }
        ).fold(
            constant('Unknown service error'), function (error) {
                return error.message;
            }
        );
    };
    ...
    var handleServiceError = function (status, blob) {
        return Utils.readBlob(blob).then(
            function (text) {
    ...
    {
        return Utils.requestUrlAsBlob(url, {}, withCredentials).then(
            function (result) {
        ...
        return function () {
            ...
            return imgOpt.fold(
                function () {
                    ...
                    return editor._scanForImages().then(
                        function () {
                            return findBlob(editor, img.dom());
                        }
                    ).then(blobToImageResult).then(fn).then(
    ...
    var rotate$2 = function (editor, imageUploadTimerState, angle) {
        return function () {
            ...
            return selectedImageOperation(
                editor, imageUploadTimerState, function (imageResult) {
    ...
    var makeOpen = function (editor, imageUploadTimerState) {
        return function () {
            ...
            var originalSizeOpt = originalImgOpt.map(
                function (origImg) {
            ...
            imgOpt.each(
                function (img) {
                    Actions.getEditableImage(editor, img.dom()).each(
                        function (_) {
                            Actions.findBlob(editor, img.dom()).then(
                                function (blob) {
